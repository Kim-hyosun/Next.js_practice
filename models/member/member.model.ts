...
async function add({ uid, email, displayName, photoURL }: InAuthUser): Promise<AddResult> {
  try {
    ...
    const addResult = await FirebaseAdmin.getInstanse().Firebase.runTransaction(async (transaction) => {
      const memberRef = FirebaseAdmin.getInstanse().Firebase.collection(MEMBER_COL).doc(uid);

      const screenNameRef = FirebaseAdmin.getInstanse().Firebase.collection(SCR_NAME_COL).doc(screenName);
...
async function findByScreenName(screenName: string): Promise<InAuthUser | null> {
  const memberRef = FirebaseAdmin.getInstanse().Firebase.collection(SCR_NAME_COL).doc(screenName);
